...
@pytest.mark.parametrize("size, axes, periodicity, expected_shape, expected_data_shape, start_coords", test_data)
def test_state_initialisation(size, axes, periodicity, expected_shape, expected_data_shape, start_coords):
...
def test_state_setters_and_copy():
    ...
    state = State(geometry=geometry, random=False, cell_keys=['alive', 'oxygen saturation'])

    state['alive'] = np.ones((3,3))
    assert np.all(state['alive'] == 1)

    state['oxygen saturation'] = np.concatenate((np.ones((2,3)), np.zeros((1,3))), axis=0)
...
test_data = [
    ...
    ((2,1,5,2), 'xyza', 'a', (4,3,7,4), (0,1,3,1), (1,1,3,0), (0,0,2,-1)),
]
@pytest.mark.parametrize("size, axes, periodicity, expected_padded_size, const_coords, wrap_pad_coords, wrap_orig_coords", test_data)
def test_state_padding(size, axes, periodicity, expected_padded_size, const_coords, wrap_pad_coords, wrap_orig_coords):
    geometry = Geometry(size=size, axes=axes, periodicity=periodicity)
